...
<?php
$flag=0;
if(isset($_POST['notice']) && !empty($_POST['notice']) && $_POST['notice']!='undefined' && $_POST['notice']!='')
		{
			
			$flag=1;
		}
		else
			echo "<span>Notification  Feild Required.</span><br>";
		
	if($flag){
	@mysql_connect(DB_HOST,DB_USER,DB_PASSWORD) or die('Could not connect.');
	@mysql_select_db(DB_DATABASE) or die('Could not connect.');
	$notice=$_POST['notice'];
	$query1=mysql_query("INSERT INTO `recent_updates` (`notice`) VALUES ('$notice')");
		if($query1)
		{
			Echo 'The Notication is added successfully.';
			
		}
		else
		{
			Echo 'All feilds Required.Some problem occured.';
...
?>
		<center>
<div class = "container">
<center><h1>Add Notification</h1></center><br>
...
<div class="col-sm-6">
<input type="text"  class="form-control" name='notice' id="notice" autocomplete='off' placeholder="Add Notification" required>
</div>
</div>
<br><br>
<div class="form-group">
